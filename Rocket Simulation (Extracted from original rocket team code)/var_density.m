...
P = 1:7 % Initializing
P(1) = P0*(T1/T0)^(-g/a0/R);
P(2) = P(1)*(T1/T0)^(-g/a0/R);
P(3) = P(2)*(T1/T0)^(-g/a0/R);
P(4) = P(3)*(T1/T0)^(-g/a0/R);
P(5) = P(4)*(T1/T0)^(-g/a0/R);
P(6) = P(5)*(T1/T0)^(-g/a0/R);
P(7) = P(6)*(T1/T0)^(-g/a0/R);
...
if h <= h_atm(1)
    i = 1;    
    T = T0 + a(i)*h;
    rhof = rho0*(T/T0)^(-g/a0/R-1);
    
elseif h_atm(1) < h <= h_atm(2)
    i = 2; 
    T = T(i-1) + a(i)*(h - h_atm(i-1));
    rhof = rho(i-1)*exp(-g/R/T(i)*(h-h_atm(i-1)));
    
elseif h_atm(2) < h <= h_atm(3) 
    i = 3; 
    T = T(i-1) + a(i)*(h - h_atm(i-1));
    rhof = rho(i-1)*(T/T(i-1))^(-g/a(i-1)/R-1);
